...
class Config:
    def __init__(self):
        try:
            ...
            self.FFMPEG = config(
                "FFMPEG",
                default='ab-av1 auto-encode -i "{}" --svt tune=0 --max-crf 70 --min-vmaf 86 --preset 7 --vfilter scale=-2:480 -o "{}"',
            )
            ...
            self.MUX_ARGS = config(
                "MUX_ARGS",
                default='-c:v copy -c:a libopus -b:a 64k -ac 2 -metadata:s:v title="" -metadata:s:a title="" -metadata:s:s title="" -metadata title=""',
            )
